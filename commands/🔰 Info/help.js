  ...
  run: async (client, message, args, cmduser, text, prefix) => {
    ...
    try {
      if (args[0]) {
        ...
        let button_tutorial = new MessageButton().setStyle('DANGER').setCustomId("4").setLabel("Look through the cmds")
        let menuOptions = [
          {
            label: "Overview",
            value: "Overview",
...
<:Insite:900389724936609842>  Made by [**Insite Development**](https://discord.gg/aqPRGvXg)`)
      ...
      function allotherembeds_eachcategory(filterdisabled = false) {
        ...
        var embed7 = new MessageEmbed()
          ...
          .addField("<:Insite:900389724936609842> **Most used Systems**", "> " + client.commands.filter((cmd) => cmd.category === "💪 Setup" && cmd.type.includes("system")).sort((a, b) => a.name.localeCompare(b?.name)).map((cmd) => `\`${cmd.name}\``).join("︲"))
